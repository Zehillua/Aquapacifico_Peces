...
import styles from './CojinovaFood.module.css';
...
const CojinovaFood = () => {
  ...
  useEffect(() => {
    const fetchStages = async () => {
      try {
        const response = await fetch('http://localhost:5000/cojinova-etapas', {
          method: 'GET',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/cojinova-food', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className={styles.cojinovaFoodContainer}>
      <img src={logo} alt="Logo AquaPacifico" className={styles.cojinovaFoodLogo} />
      <div className={styles.cojinovaFoodMain}>
        <div className={styles.cojinovaFoodContent}>
          <form className={styles.cojinovaFoodForm} onSubmit={handleSubmit}>
            <div className={styles.cojinovaFoodFormGroup}>
            ...
            <button type="submit" className={styles.cojinovaFoodButton} disabled={isLoading}>
              {isLoading ? 'Cargando...' : 'Enviar'}
            </button>
          </form>
          <button
            type="button"
            className={styles.cojinovaFoodBackButton}
            onClick={() => navigate('/cojinova')}
            disabled={isLoading}
          >
            {isLoading ? 'Cargando...' : 'Volver'}
          </button>
        </div>
        <div className={styles.cojinovaFoodDetails}>
        <div className={styles.cojinovaFoodDetailBox}>
              <h3>% de Levadura</h3>
