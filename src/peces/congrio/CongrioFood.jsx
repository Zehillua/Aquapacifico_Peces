...
import styles from './CongrioFood.module.css';
...
const CongrioFood = () => {
  ...
  useEffect(() => {
    const fetchStages = async () => {
      try {
        const response = await fetch('http://localhost:5000/congrio-etapas', {
          method: 'GET',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/congrio-food', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className={styles.congrioFoodContainer}>
      <img src={logo} alt="Logo AquaPacifico" className={styles.congrioFoodLogo} />
      <div className={styles.congrioFoodMain}>
        <div className={styles.congrioFoodContent}>
          <form className={styles.congrioFoodForm} onSubmit={handleSubmit}>
            <div className={styles.congrioFoodFormGroup}>
            ...
            <button type="submit" className={styles.congrioFoodButton} disabled={isLoading}>
              {isLoading ? 'Cargando...' : 'Enviar'}
            </button>
          </form>
          <button
            type="button"
            className={styles.congrioFoodBackButton}
            onClick={() => navigate('/congrio')}
            disabled={isLoading}
          >
            {isLoading ? 'Cargando...' : 'Volver'}
          </button>
        </div>
        <div className={styles.congrioFoodDetails}>
        <div className={styles.congrioFoodDetailBox}>
              <h3>% de Levadura</h3>
